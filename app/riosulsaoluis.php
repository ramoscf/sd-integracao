...
try{
	##FIREBIRD
	$con1 = new PDO('firebird:dbname=riosulvilasaoluis.ddns.com.br/3050:/SDSuper/Dados/SDSuper.fdb', 'CONSULTORIA', 'CONSULTA321');
...
//
   ...
   while ($row = $sql_stmt2->fetch())
   {
	...
	$dataalteracao = $row[2];
	$sku = $row[3];
	if($row[4] == '11'){$idgrupo = 'IMPORTACAO';}
	if($row[4] == '22'){$idgrupo = 'SEARA';}
	...
	if($row[5] == '21'){$proporcao = 'CX';}
	if($row[5] == '31'){$proporcao = 'M';}
	if($row[5] == '41'){$proporcao = 'L';}
	if($row[5] == '51'){$proporcao = 'FARDO';}
	if($row[5] == '62'){$proporcao = 'EMBALAGEM';}
	if($row[5] == '20'){$proporcao = 'CENTIMETRO';}


	##MYSQL
	$con2 = new PDO('mysql:host=cartazfacilpro.ctj8bnjcqdvd.us-east-2.rds.amazonaws.com;dbname=riosulvilasaoluis','cartazdb', 'tbCJShR2');

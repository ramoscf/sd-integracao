...
try{
	##FIREBIRD
	$con1 = new PDO('firebird:dbname=168.228.40.182:/SDSuper/Dados/SDSuper.fdb', 'CONSULTORIA', 'CONSULTA321');
	$con1->query("SET CHARACTER SET utf8");
	$sql_stmt2 = $con1->prepare("SELECT WIDPRODUTO, WNOMEGONDOLA, WDATAULTIMAALTERACAO, WCODIGOPRINCIPAL, WIDPRODUTOGRUPO, WIDUNIDADE FROM PRODUTOS WHERE WDATAULTIMAALTERACAO >= '$dataFormat'");
   ...
   while ($row = $sql_stmt2->fetch())
   {
	...
	$dataalteracao = $row[2];
	$sku = $row[3];
	if($row[4] == '1621'){$idgrupo = 'PATRIMONIAL';}
	if($row[4] == '1631'){$idgrupo = 'PERECIVEIS';}
	if($row[4] == '1641'){$idgrupo = 'MERCEARIA';}
	if($row[4] == '1651'){$idgrupo = 'LEITES';}
	if($row[4] == '1661'){$idgrupo = 'CEREAIS';}
	if($row[4] == '1671'){$idgrupo = 'USO E CONSUMO';}
	if($row[4] == '1611'){$idgrupo = 'A CLASSIFICAR';}
	...
	if($row[5] == '11'){$proporcao = 'KG';}
	if($row[5] == '20'){$proporcao = 'CENTIMETRO';}
	if($row[5] == '21'){$proporcao = 'KGS';}
	if($row[5] == '31'){$proporcao = 'PC';}

	##MYSQL
	$con2 = new PDO('mysql:host=cartazfacilpro.ctj8bnjcqdvd.us-east-2.rds.amazonaws.com;dbname=riosulqueimados','cartazdb', 'tbCJShR2');

...
try{
	##FIREBIRD
	$con1 = new PDO('firebird:dbname=riosulvilanova.ddns.com.br/3050:/SDSuper/Dados/SDSuper.fdb', 'CONSULTORIA', 'CONSULTA321');
   ...
   while ($row = $sql_stmt2->fetch())
   {
	...
	if($row[5] == '21'){$proporcao = 'ML';}
	if($row[5] == '31'){$proporcao = 'GR';}

	if($row[5] == '41'){$proporcao = 'LT';}

	if($row[5] == '51'){$proporcao = 'KGG';}

	if($row[5] == '61'){$proporcao = 'CXX';}
	if($row[5] == '71'){$proporcao = 'PECA';}


	##MYSQL
	$con2 = new PDO('mysql:host=cartazfacilpro.ctj8bnjcqdvd.us-east-2.rds.amazonaws.com;dbname=riosulmiguelpereira','cartazdb', 'tbCJShR2');
...
try{
	##FIREBIRD
	$con1 = new PDO('firebird:dbname=179.124.184.75:/SDSuper/Dados/SDSuper.fdb', 'CONSULTORIA', 'CONSULTA321');
	$con1->query("SET CHARACTER SET utf8");
